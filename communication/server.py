...
class Server:
    ...
    def initialize_client(self, client):
        ...
        self.print_message(
            CLI.message(
                f"Client Connected: {str(client.getpeername())}", "green", verbose=False
            )
        )
        return client_node
    ...
    def show_clients(self):
        if len(self.clients) == 0:
            self.print_message(
                CLI.message("NO CLIENTS CONNECTED", "red", verbose=False)
            )
            return
    ...
    def run(self):
        ...
        self.print_message(
            CLI.message(
                f"SERVER STARTED {self.host}:{self.port}", "green", verbose=False
            )
        )
        self.server.listen()
        self.threads.extend(
