...
class Node:
    def __init__(self, 
        ...
        self.network = ip.IPv4Network(f"{'.'.join(str(self.IP).split('.')[:-1])}.0/{str(self.net_mask[1])}")
        self.neighbors = []
        self.tags = tags
        self.help = HelpMenu(self, generic_data_keys = [
            'ID',
            'IP',
            'net_mask',
            'tags',
...
class HelpMenu:
    ...
    def __init__(self, parent, generic_data_keys = []):
        self.parent = parent
        self.compact = False
        self.generic_data = {k: v for k, v in vars(self.parent).items() if k in generic_data_keys}

    def show(self, compact:bool = True, basic:bool = False, verbose:bool = True) -> str:
        ...
        return table(self.get_attributes(data), headers=self.get_headers(compact), indent_level=1, verbose=verbose)
    ...
    def handle_list(self, attr: str, value: list) -> list:
        if not self.compact:
            list_attributes = [self.get_list_attribute(attr, f"[{len(value)}]" if not self.compact else str(value))]
            if len(value) == 0:
                list_attributes.append(self.get_item_attributes(f"⠀{'⠀':>{HelpMenu.SPACING/2+2}} {f'[] ──╯'}", f'╰─── Empty List'))
            else:
                for i in range(len(value)):
                    list_attributes.append(self.get_item_attributes(f"⠀{'⠀':>{HelpMenu.SPACING/2+1}} {f'[{str(i)}] ──╯'}", f'╰─>  {value[i]}'))
        else:
            list_attributes = [self.get_standard_attribute(attr, f"[{len(value)}]" if not self.compact else str(value))]

        return list_attributes
    
    def handle_dict(self, attr: str, value: dict) -> list:
        dict_attributes = [self.get_standard_attribute(attr, "[...]")]
        attributes = [self.get_standard_attribute(f"{'-':>20} {key:<15} =", self.get_name_or_val(val)) for key, val in value.items()]
        return dict_attributes + attributes


    def get_standard_attribute(self, attr:str, value:any) -> list[str, str]:
        return [f"{attr:<{int(HelpMenu.SPACING/2 + 9)}} =", f"{str(value):<{int(HelpMenu.SPACING/2)}}{'⠀'}"]
    
    def get_list_attribute(self, attr:str, value:any) -> list[str, str]:
        return [f"{attr:<{int(HelpMenu.SPACING/2)}} {'Index ──╮' if not self.compact else '='}", f"╭── Values {str(value):<{int(HelpMenu.SPACING/2)}}{'⠀'}"]
